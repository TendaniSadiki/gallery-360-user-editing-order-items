...
import { getItemDeliveryPrice, getParcelDeliveryPrice } from "../../../services/courier-guy";
import { createDecimalNumber } from "../../../utils/helper-functions";
...
export default function CartScreen() {
    ...
    await firestore.collection('cartItem').doc(uid).get().then(doc => {
      if(doc.exists) {
        ...
        firestore.collection('users').doc(uid).get().then(async userDoc => {
          ...
          Promise.all(artistItems.map(async artistItem => {
            // console.log({ items: artistItem.cartItems });
            
            return getArtistPackagesDeliveryPrice(userData, artistItem, artistItem.cartItems)
          })).then(resultArray => {
            console.log({ res: resultArray } );
            let newArtistItems = artistItems
            let totalDeliveryPrice = 0
            resultArray.forEach(priceItem => {
              console.log({ priceItem });
              // const priceItemData = priceItem.data.
              console.log(priceItem.data.rates[0].rate);
              
              totalDeliveryPrice += Number(priceItem.data.rates[0].rate)
              console.log({ totalDeliveryPrice });
              
              newArtistItems = newArtistItems.map(item => {
                if(priceItem.artistUid === item.artistUid) {
                  return {
                    ...item,
                    deliveryPrice: createDecimalNumber(priceItem.data.rates[0].rate),
                    shipmentDetails: priceItem.data.rates[0]
                  } 
                } else {
                    return item
                }
            ...
            firestore.collection('cartItem').doc(uid).update({
              ...
              deliveryTotal: createDecimalNumber(totalDeliveryPrice),
              grandTotal: createDecimalNumber(totalDeliveryPrice + Number(doc.data().subtotal))
            }).then(res => {
              setIsCalculatingPrice(false)
              router.navigate({ pathname: '/payments' })
            }).catch(err => {
              console.log(err);
  ...
  const getArtistPackagesDeliveryPrice= async(userData, artistItem, parcels) => {
    console.log({ userData, artistItem });
    console.log({ lat: artistItem.collection_address.coords.lat });
    console.log({ parcels: parcels });
    
    
    return getParcelDeliveryPrice(artistItem, userData, parcels)
  } 
  useEffect(() => {
    ...
    return (
      <View style={globalStyles.flatlistView}>
        ...
        <View style={globalStyles.priceContainer}>
          ...
          <Text style={globalStyles.priceTxt}>{`R${price.toFixed(2)}`}</Text>
        </View>
      </View>
    );
  };
